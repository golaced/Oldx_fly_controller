...
void mavlink_msg_statustext_send(mavlink_channel_t chan, uint8_t severity, char *text)
{
	...
	mav_array_memcpy(packet.text, text, sizeof(char)*50);
#if MAVLINK_CRC_EXTRA
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_STATUSTEXT, (char *)&packet, MAVLINK_MSG_ID_STATUSTEXT_LEN, MAVLINK_MSG_ID_STATUSTEXT_CRC);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_STATUSTEXT, (char *)&packet, MAVLINK_MSG_ID_STATUSTEXT_LEN);
#endif
#endif
}
...
{
	return 0;//_MAV_RETURN_char_array(msg, text, 50,  1);
}
