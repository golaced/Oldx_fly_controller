 ...
 */
uint16_t _pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
 ...
 * @param vfr_hud C-struct to read the message contents from
 */
uint16_t _encode_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, const mavlink_vfr_hud_t* vfr_hud)
{
	return _pack_chan(system_id, component_id, chan, msg, vfr_hud->airspeed, vfr_hud->groundspeed, vfr_hud->heading, vfr_hud->throttle, vfr_hud->alt, vfr_hud->climb);
}
...
void _send(mavlink_channel_t chan, float airspeed, float groundspeed, int16_t heading, uint16_t throttle, float alt, float climb)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_VFR_HUD_LEN];
	_mav_put_float(buf, 0, airspeed);
 ...
 */
float _get_airspeed(const mavlink_message_t* msg)
{
 ...
 */
float _get_groundspeed(const mavlink_message_t* msg)
{
 ...
 */
int16_t _get_heading(const mavlink_message_t* msg)
{
 ...
 */
uint16_t _get_throttle(const mavlink_message_t* msg)
{
 ...
 */
float _get_alt(const mavlink_message_t* msg)
{
 ...
 */
float _get_climb(const mavlink_message_t* msg)
{
